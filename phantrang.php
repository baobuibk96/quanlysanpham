...
<?php
  ...
  //-------------------------------------------------
  $qr=mysqli_query($link, "select * from sanpham");//query den bang co ten la: test trong CSDL
  $n_record=mysqli_num_rows($qr);//so ban ghi cua bang
  ...
  if ($n!='all')
  {
    $qr1=mysqli_query($link, "select * from test limit $s,$p") or die (mysql_error());
  }
  else
  {
    $qr1=mysqli_query($link, "select * from test");
  }
  //--------Hien thi bang-----------------------------
  view_page();
